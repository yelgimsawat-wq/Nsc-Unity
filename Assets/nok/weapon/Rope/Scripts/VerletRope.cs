...
public class VerletRope : MonoBehaviour
{
    ...
    [Tooltip("ตัวหน่วงพลังงานของเชือกต่อสเต็ป ค่าใกล้ 1 = แกว่งเหมือนเดิมแทบไม่หยุด, ค่าต่ำลง (เช่น 0.9-0.95) = เชือกหยุดแกว่ง/หยุดเหวี่ยงเป็นวงกลมได้เร็วขึ้น")]
    [Range(0f, 1f)] [SerializeField] private float m_Damping = 0.97f;
    ...
    private void CalculateNewPositions(float deltaTime)
    {
        ...
        for (int i = 0; i < m_VerletNodes.Length; i++)
        {
            ...
            // เพิ่ม m_Damping คูณเข้ากับ "ความเร็ว" (Position - PrevoiusPosition)
            // เพื่อให้พลังงานในระบบค่อยๆหายไปทุกสเต็ป แทนที่จะคงอยู่เต็มๆแบบเดิม
            // ซึ่งเป็นสาเหตุที่เชือกเหวี่ยงเป็นวงกลมแล้วไม่ยอมหยุด
            var velocity = (currNode.Position - currNode.PrevoiusPosition) * m_Damping;
            var newPosition = currNode.Position + velocity + gravityStep;
