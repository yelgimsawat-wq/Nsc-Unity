...
#if UNITY_WEBGL
    // UNIMIC_WEBGL_SHIM
    // Browsers expose no capture device to Unity, so UnityEngine.Microphone is
    // stripped from the WebGL player entirely and every call below fails to
    // compile. This namespace-local stand-in resolves those calls and reports
    // "no devices, never recording", which is exactly what the platform is.
    // Voice chat therefore no-ops on web while every other platform is untouched.
    internal static class Microphone {
        public static string[] devices { get { return new string[0]; } }
        public static void GetDeviceCaps(string device, out int minFreq, out int maxFreq) { minFreq = 0; maxFreq = 0; }
        public static AudioClip Start(string device, bool loop, int lengthSec, int frequency) { return null; }
        public static bool IsRecording(string device) { return false; }
        public static void End(string device) { }
        public static int GetPosition(string device) { return 0; }
    }
#endif
}
